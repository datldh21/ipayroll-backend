...
const timekeeping_entity_1 = require("../entities/timekeeping.entity");
const si_monthly_record_entity_1 = require("../entities/si-monthly-record.entity");
const si_rate_version_entity_1 = require("../entities/si-rate-version.entity");
const si_rate_entity_1 = require("../entities/si-rate.entity");
const tax_bracket_version_entity_1 = require("../entities/tax-bracket-version.entity");
const tax_bracket_entity_1 = require("../entities/tax-bracket.entity");
const payroll_config_entity_1 = require("../entities/payroll-config.entity");
const working_day_calendar_entity_1 = require("../entities/working-day-calendar.entity");
const variable_income_entity_1 = require("../entities/variable-income.entity");
const payroll_batch_entity_1 = require("../entities/payroll-batch.entity");
...
async function runSeed() {
    const dataSource = new typeorm_1.DataSource({
        ...
        entities: [
            ...
            timekeeping_entity_1.Timekeeping,
            si_monthly_record_entity_1.SiMonthlyRecord,
            si_rate_version_entity_1.SiRateVersion,
            si_rate_entity_1.SiRate,
            tax_bracket_version_entity_1.TaxBracketVersion,
            tax_bracket_entity_1.TaxBracket,
            payroll_config_entity_1.PayrollConfig,
            working_day_calendar_entity_1.WorkingDayCalendar,
            variable_income_entity_1.VariableIncome,
            payroll_batch_entity_1.PayrollBatch,
    ...
    const contractRepo = dataSource.getRepository(employee_contract_entity_1.EmployeeContract);
    const timekeepingRepo = dataSource.getRepository(timekeeping_entity_1.Timekeeping);
    const siRecordRepo = dataSource.getRepository(si_monthly_record_entity_1.SiMonthlyRecord);
    const siVersionRepo = dataSource.getRepository(si_rate_version_entity_1.SiRateVersion);
    const siRateRepo = dataSource.getRepository(si_rate_entity_1.SiRate);
    const taxVersionRepo = dataSource.getRepository(tax_bracket_version_entity_1.TaxBracketVersion);
    const taxBracketRepo = dataSource.getRepository(tax_bracket_entity_1.TaxBracket);
    const configRepo = dataSource.getRepository(payroll_config_entity_1.PayrollConfig);
    const calendarRepo = dataSource.getRepository(working_day_calendar_entity_1.WorkingDayCalendar);
    const variableRepo = dataSource.getRepository(variable_income_entity_1.VariableIncome);
    const batchRepo = dataSource.getRepository(payroll_batch_entity_1.PayrollBatch);
    const recordRepo = dataSource.getRepository(payroll_record_entity_1.PayrollRecord);
    const proposalRepo = dataSource.getRepository(proposal_entity_1.Proposal);
    ...
    const taxVersion = await taxVersionRepo.save(taxVersionRepo.create({
    ...
    for (let i = 0; i < bracketData.length; i++) {
        ...
        const bracket = new tax_bracket_entity_1.TaxBracket();
        bracket.versionId = taxVersion.id;
        bracket.bracketOrder = i + 1;
        bracket.incomeFrom = row[0];
        bracket.incomeTo = row[1];
        bracket.rate = row[2];
        await taxBracketRepo.save(bracket);
    }
    const siVersion = await siVersionRepo.save(siVersionRepo.create({
    ...
    for (const [siType, party, rate] of siRates) {
        const sr = new si_rate_entity_1.SiRate();
    ...
    for (const [month, year, std] of months) {
        await calendarRepo.save(calendarRepo.create({ year, month, standardDays: std }));
    }
    const empData = [
        [
            'EMP001',
    ...
    for (const d of empData) {
        const emp = await employeeRepo.save(employeeRepo.create({
            employeeCode: d[0],
            fullName: d[1],
            email: d[2],
            phone: d[3],
            ...
            orgLevel5: '',
            status: d[10],
            onboardDate: d[11] ? new Date(d[11]) : null,
            officialDate: d[12] ? new Date(d[12]) : null,
            lastWorkingDate: d[13] ? new Date(d[13]) : null,
            costAccount: d[15],
        }));
        employees.push(emp);
        await contractRepo.save(contractRepo.create({
            employeeId: emp.id,
            contractType: emp.status === 'thu_viec' || emp.status === 'nghi_viec_tv'
                ? 'hdtv'
                : 'hdld',
            baseSalary: d[9],
    ...
    const empBaseMap = new Map(empData.map((d) => [d[0], d[9]]));
    const empDependentsMap = new Map(empData.map((d) => [d[0], d[14]]));
    const batchMonths = [
        [10, 2025],
        [11, 2025],
        [12, 2025],
    ...
    for (const emp of employees) {
        for (const [month, year] of batchMonths) {
            ...
            if (!tk) {
                ...
                else if (emp.status === 'het_thu_viec' &&
                    !(month === 2 && year === 2026))
                    tk = {
                        actual: std,
                        prob: std,
            ...
            await timekeepingRepo.save(timekeepingRepo.create({
                employeeId: emp.id,
                year,
                month,
                standardDays: std,
    ...
    for (const emp of employees) {
        ...
        const isExempt = ['thai_san', 'thu_viec', 'nghi_viec_tv'].includes(emp.status);
        let bhxhE = 0, bhytE = 0, bhtnE = 0, bhxhEr = 0, bhytEr = 0, bhtnEr = 0, unionFee = 0;
        if (!isExempt) {
        ...
        await siRecordRepo.save(siRecordRepo.create({
            employeeId: emp.id,
            year: 2026,
            month: 2,
            siBase: realSiBase,
    ...
    for (const [month, year] of batchMonths) {
        ...
        for (const emp of employees) {
            ...
            await variableRepo.save(variableRepo.create({
                employeeId: emp.id,
                year,
                month,
    ...
    for (const [month, year] of batchMonths) {
        ...
        for (const emp of employees) {
            ...
            const proratedBase = prorate(base, std, probDays) +
                prorate(base, std, offDays) +
                prorate(base, std, leaveDays);
            const totalLunch = prorate(pkgLunch, std, probDays) +
                prorate(pkgLunch, std, offDays) +
                prorate(pkgLunch, std, leaveDays);
            const totalPhone = emp.status === 'thu_viec' || emp.status === 'nghi_viec_tv'
                ? 0
                : pkgPhone;
            const proratedPerf = prorate(probPkgPerf, std, probDays) +
                prorate(pkgPerf, std, offDays) +
                prorate(pkgPerf, std, leaveDays);
            const proratedTotal = proratedBase + totalLunch + totalPhone + proratedPerf;
            const totalVar = vi
            ...
            const taxMethod = getTaxMethod(emp.status);
            let pit = 0;
            let taxAssessable = 0;
            let personalDed = 0;
            let dependentDed = 0;
            ...
            if (taxMethod === 'progressive') {
                ...
                taxAssessable = Math.max(0, taxableIncome - siEmployee - personalDed - dependentDed);
        ...
        const batch = await batchRepo.save(batchRepo.create({
            ...
            totalSiEmployee: records.reduce((s, r) => s + Number(r.siEmployeeTotal || 0), 0),
            totalEmployerCost: records.reduce((s, r) => s + Number(r.totalEmployerCost || 0), 0),
            createdBy: user1.id,
            approvedBy: month === 2 && year === 2026 ? null : user2.id,
            approvedAt: month === 2 && year === 2026 ? null : new Date(year, month - 1, 27),
        }));
